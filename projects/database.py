...
class DB:
    ...
    def connect(self, database=None):
        ...
        try:
            ...
            connection = pymysql.connect(
                host=self._host, user=self._username, password=self._password, database=self._database
            )
            return connection
    ...
    def drop_database(self, database=None):
        ...
        if database:  # database if not null
            self._database = database
        query = (
            self.DROP_DATABASE_QUERY + self._database
        )  # see self.DROP_DATABASE_QUERY at the beginning of the class
    ...
    def get_user_id(self, query, values=None):
        user_id = self._execute_query(query, values=values, action=self.SELECT)
        return user_id
